...
public class LoginController {
     ...
     * - 检验两次密码是否相同(前后端都要校验)
     * 
     * @param registerRequest 包含email、nickname、password、confirmPassword
     * @return 注册结果
     */
    ...
    @PostMapping("/forget")
    public Response forgetPassword(@RequestBody EmailRequest emailRequest){
        userService.forgetPassword(emailRequest.getEmail());
        return Response.success();
    }
    ...
    @PutMapping("/forget")
    public Response forgetPasswordUpdate(@RequestBody PasswordView passwordView){
        userService.forgetPasswordUpdate(passwordView);
        return Response.success();
    }
